...
private:
    ...
    ImguiThumbnail toImguiThumbnail(const TextureResource& texture) {
    ...
    ImguiThumbnail thumbnail_PreviewNotAvailable() {
        const TextureResource* texture = assetLoader_.get<TextureResource>(PREVIEW_NOT_AVAILABLE_PATH);
    ...
    std::optional<ImguiThumbnail> getTexturePreview(const std::string& assetName) {
        const TextureResource* texture = assetLoader_.get<TextureResource>(assetName);
