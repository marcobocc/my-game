...
#include <vector>
#include <volk.h>
#include "systems/rendering/vulkan/core/utils/buffers.hpp"
#include "systems/rendering/vulkan/core/utils/memory.hpp"
...
public:
    ...
    ~VulkanPickingBackend() { destroyBuffer(context_.device, readbackBuffer_); }
    ...
    void processReadback(const std::vector<std::string>& objectIdMap) {
    ...
    std::optional<std::string> takePendingResult() { return std::exchange(resolvedResult_, std::nullopt); }
