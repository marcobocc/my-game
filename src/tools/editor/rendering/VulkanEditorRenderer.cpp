...
VulkanEditorRenderer::VulkanEditorRenderer(GameWindow& window,
...
RenderTargetHandle VulkanEditorRenderer::createRenderTarget(uint32_t width, uint32_t height) {
...
void VulkanEditorRenderer::destroyRenderTarget(RenderTargetHandle handle) {
    vkDeviceWaitIdle(context_.device);
...
VkDescriptorSet VulkanEditorRenderer::getRenderTargetImGuiId(RenderTargetHandle handle) const {
    auto it = renderTargetImGuiSets_.find(handle.index);
    return (it != renderTargetImGuiSets_.end()) ? it->second : VK_NULL_HANDLE;
}

VulkanRenderTarget VulkanEditorRenderer::getRenderTarget(RenderTargetHandle handle) const {
...
void VulkanEditorRenderer::executeRenderGraph(VkCommandBuffer cmd,
                                              VkImage colorImage,
...
void VulkanEditorRenderer::setupRenderGraph(VkFormat colorFormat, VkImageUsageFlags colorUsage, VkExtent2D extent) {
